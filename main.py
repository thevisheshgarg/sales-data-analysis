...
auth_provider = PlainTextAuthProvider('opBBvTaYPDFxFoSiWNXtbUgK',
                                      'LTTr-a,MS66-7uaz7URldCxtgfJ+fkTQi+rFxhZGZ4PQiYM8KhTQhR6ireKKSeTsmFmu.zC7Z0Hz8eZTJ7MUGuZgJ,Hr2OwwbLdoN.YF2Wbs-B4JD8uApsQZvMGvqk9h')
cluster = Cluster(protocol_version=3, cloud=cloud_config, auth_provider=auth_provider)
session = cluster.connect()


session.execute("USE storesalespredictor")
...
def storeData(pl):
    ...
    insert = "INSERT INTO storesalespredictor.Sales ({}) VALUES ({});".format(column, value)
    session.execute("USE storesalespredictor")
